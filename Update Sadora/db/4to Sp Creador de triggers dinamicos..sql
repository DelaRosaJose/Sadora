...
DECLARE @sql varchar(8000), @TABLE_NAME sysname, @CountTablas int, @CountPosition int = 1
SET NOCOUNT ON
...
FROM	INFORMATION_SCHEMA.Tables a join 
		INFORMATION_SCHEMA.COLUMNS b on a.TABLE_NAME = b.TABLE_NAME JOIN
		(select * from sys.all_objects where type = 'u') C on a.TABLE_NAME = c.name full outer join
		(select * from sys.all_objects where type = 'tr') d on c.object_id = d.parent_object_id
WHERE	TABLE_TYPE='BASE TABLE' AND a.TABLE_NAME not in ('sysdiagrams', 'Audit') and b.COLUMN_NAME = 'UsuarioID' and d.type_desc is null


select @CountTablas = (select count(*) from @TableNam);

select * from @TableNam 

while @CountTablas >= @CountPosition
begin

select @TABLE_NAME = (select Table_Name from @TableNam where Id = @CountPosition);
